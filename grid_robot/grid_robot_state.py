class grid_robot_state:
    ...
    def is_goal_state(_grid_robot_state):
        ...
        # Check if the robot is at the lamp's location
        at_lamp_location = robot_location[0] == lamp_location[0] and robot_location[1] == lamp_location[1]

        # Check if the robot is empty-handed (carried_stairs is 0)
        robot_empty_handed = _grid_robot_state.get_carried_stairs() == 0

        # Debugging outputs
        print(f"Robot Location: {robot_location}, Lamp Location: {lamp_location}")
        print(f"Robot Empty-Handed: {robot_empty_handed}")

        # The goal is achieved when the robot is at the lamp location and it is empty-handed
        return at_lamp_location and robot_empty_handed
        ...
        for dx, dy in directions:
            ...
            if 0 <= new_x < len(self.map) and 0 <= new_y < len(self.map[0]):
                if self.map[new_x][new_y] != -1:  # Check for valid cell (not an obstacle)
                    cost = 1
                    if self.lamp_height > 0:  # Additional cost if carrying stairs
